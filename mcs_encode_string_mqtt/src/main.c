...
void mcs_mqtt_callback(char *rcv_buf) {
    char *arr[3];
    char *del = ",";
    mcs_splitn(arr, rcv_buf, del, 3);
    // 1459307476444,encodeByMD5,test
    if (0 == strncmp (arr[1], ENCODE_MD5_CHANNEL, strlen(ENCODE_MD5_CHANNEL))) {
        /* encode BY MD5 */
        uint8_t digest[HAL_MD5_DIGEST_SIZE] = {0};
        printf("User give: %s \n", arr[2]);
        hal_md5_context_t context = {0};
        hal_md5_init(&context);
        hal_md5_append(&context, arr[2], strlen(arr[2]));
